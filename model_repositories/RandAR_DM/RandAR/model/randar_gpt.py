...
class RandARTransformer(nn.Module):
    ...
    def forward(
        ...
        if idx is not None and cond_idx is not None:
            return self.forward_train(begin, idx, cond_idx, token_order, input_pos, targets, mask, valid)
        else:
    ...
    def forward_train(self,
        ...
        first_token_embeddings = token_embeddings[:, :begin]
        last_token_embeddings = token_embeddings[:, begin:]
        first_position_instruction_tokens = position_instruction_tokens[:, :begin]
        last_position_instruction_tokens = position_instruction_tokens[:, begin:]

        h = torch.cat(
            (cond_embeddings, interleave_tokens(first_position_instruction_tokens, first_token_embeddings)),
            dim=1
        )

        length = h.shape[-2]

        h = torch.cat((h, last_position_instruction_tokens), dim=1)
        ...
        first_token_freqs_cis = token_freqs_cis[:, :begin]
        last_token_freqs_cis = token_freqs_cis[:, begin:]

        freqs_cis = torch.cat(
            (self.freqs_cis[:self.cls_token_num].unsqueeze(0).repeat(bs, 1, 1, 1), interleave_tokens(first_token_freqs_cis, first_token_freqs_cis)),
            dim=1
        )
        freqs_cis = torch.cat((freqs_cis, last_token_freqs_cis), dim=1)
        ...
        # print(h.shape[-2], length)

        mask = torch.ones(h.shape[-2], h.shape[-2], dtype=torch.bool).tril(diagonal=0).to(h.device)
        mask[:, length:] = 0
        mask += torch.diag(torch.ones(h.shape[-2], dtype=torch.bool)).to(h.device)
        ...
        return token_logits, loss, token_order, targets
